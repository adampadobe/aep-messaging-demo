...
// MARK: - FlynasLiveActivityView

@available(iOS 16.1, *)
struct FlynasLiveActivityView: View {
    @State private var runningActivities: [Activity<FlynasFlightAttributes>] = []
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                if #available(iOS 17.2, *) {
                    PushToStartSection<FlynasFlightAttributes>(
                        pushToStartToken: TokenCollector.flynasFlightPushToStartToken
                    )
                } else {
            ...
            .navigationTitle("✈️ Flynas Flight")
            .padding(.horizontal, 10)
...
@available(iOS 16.1, *)
private extension FlynasLiveActivityView {
    var startActivitySection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            SectionDescription(text: "Start a Flynas flight Live Activity. This shows flight status, Wi-Fi availability, and real-time updates.")
    ...
    func activityRow(activity: Activity<FlynasFlightAttributes>, index: Int) -> some View {
    ...
    func refreshActivities() {
        runningActivities = Activity<FlynasFlightAttributes>.activities
    }
    ...
    func startLiveActivity() {
        ...
        let attributes = FlynasFlightAttributes(
        ...
        let initialContentState = FlynasFlightAttributes.ContentState(
            status: "Departed",
            wifiAvailable: true,
        ...
        do {
            let newActivity = try Activity<FlynasFlightAttributes>.request(
                attributes: attributes,
                contentState: initialContentState,
                pushType: .token
            )

            print("Flynas Live Activity requested. ID: \(newActivity.id)")
            refreshActivities()
        } catch {
    ...
    func startChannelActivity() {
        ...
        do {
            if #available(iOS 18.0, *) {
                ...
                print("Flynas Live Activity (CHANNEL: \(trimmedChannelID)) requested. ID: \(newActivity.id)")
            }
            refreshActivities()
        } catch {
            print("Error requesting live activity: \(error.localizedDescription)")
    ...
    func endSelectedActivity(activity: Activity<FlynasFlightAttributes>, immediate: Bool) {
        Task {
            if immediate {
